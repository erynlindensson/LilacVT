    ...
    while repeat or input_reader.is_open():
        ...
        try:
            ...
            for face_num, f in enumerate(faces):
                ...
                packet.extend(bytearray(struct.pack("f", float(np.squeeze(f.translation)[0]))))
                packet.extend(bytearray(struct.pack("f", float(np.squeeze(f.translation)[1]))))
                packet.extend(bytearray(struct.pack("f", float(np.squeeze(f.translation)[2]))))
                if log is not None:
                    log.write(f"{frame_count},{now},{width},{height},{fps},{face_num},{f.id},{f.eye_blink[0]},{f.eye_blink[1]},{f.conf},{f.success},{f.pnp_error},{f.quaternion[0]},{f.quaternion[1]},{f.quaternion[2]},{f.quaternion[3]},{f.euler[0]},{f.euler[1]},{f.euler[2]},{f.rotation[0]},{f.rotation[1]},{f.rotation[2]},{float(np.squeeze(f.translation)[0])},{float(np.squeeze(f.translation)[1])},{float(np.squeeze(f.translation)[2])}")
                for (x,y,c) in f.lms:
